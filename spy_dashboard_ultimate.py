...
def load_data():
    ...
    vwap = VolumeWeightedAveragePrice(high=df['High'].squeeze(), low=df['Low'].squeeze(), close=df['Close'].squeeze(), volume=df['Volume'].squeeze())
    df['vwap'] = vwap.vwap()
    df['atr'] = AverageTrueRange(high=df['High'].squeeze(), low=df['Low'].squeeze(), close=df['Close'].squeeze()).average_true_range()
